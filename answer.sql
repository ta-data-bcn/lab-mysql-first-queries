...
#the best rated apps are not even close in the rating count to the most rated, telling us that above certain amounts of ratings, it's neraly imposible
#to have a 5 start rating. On the other hand we can also see that most of the best rated apps are games. This can bring us to think
#that people tend to rate games higher because they're trivial, not needed in a specific moment when if there's a bug it affects your "real" live
...
#although we see again that a lot of them are games. people are used to paying for games, we don't have enough info to answer the question yet.
#3
select track_name, rating_count_tot, user_rating, price
from data
where price != 0 
order by rating_count_tot desc
limit 10;
#here we see that out of the 10 best rated paid apps, only 3 have got a 5 rating score, this might be a hint that price influences the rating
#we are seeing a tendency to not rate 5 starts to paid apps (might be 0,5 stars the prize you need to pay to make your app paid?)we don't have enough info to answer the question yet.
#4
select track_name, rating_count_tot, user_rating, price
from data
where price != 0 
...
#this might be because 5$ apps are already much more specific and have smaller niche market and they're les penalized by puting a price on their app.
#Final
#After running some queries (Would need many more and more data to have definite conclusions) I think we can say that price can influence on and app's rating
#Although we have seen a stronge correlation between number of ratings and ratings score than we have seen between rating score and it being a paid app.
